...
import { List, ListItem, ListItemIcon, ListItemText, makeStyles, Paper, Toolbar, Typography } from "@material-ui/core";
import {
      ...
      <Paper>
        ...
        <List>
          {projects.map((project) =>
            <ListItem button onClick={() => router.push(`/${org._id}/project/${project._id}`)}>
              <ListItemIcon>
                <Folder />
              </ListItemIcon>
              <ListItemText
...
const getProjects = (oid) => {
  return fetch(`${process.env.NEXT_PUBLIC_DOMAIN}/api/v1/organization/${oid}/project`, {
    method: "GET",
  })
    .then((res) => res.json())
    .then((res) => res.result);
